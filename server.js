...
// ===== SESSION - QUAN TRỌNG =====
app.use(session({
    secret: 'evokey_super_secret_2024',
    resave: false,
...
app.post('/api/auth', (req, res) => {
    ...
    if (username === 'lichcuto' && password === 'tinkm2009') {
        ...
        return res.json({
            success: true,
            user: { username: 'lichcuto', isAdmin: true, balance: 999999, keys: [] }
        });
    }
...
app.post('/api/purchase', (req, res) => {
    ...
    // Admin: nếu không đủ tiền thì vẫn mua được
    if (username === 'lichcuto') {
        if (user.balance >= pkgInfo.price) {
            user.balance -= pkgInfo.price;
        }
        // Nếu không đủ, không trừ, vẫn tạo key
    } else {
        if (user.balance < pkgInfo.price) {
